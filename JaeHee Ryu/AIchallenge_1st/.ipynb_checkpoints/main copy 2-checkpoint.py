...
def convert_tokens(uuids, tokens, qa_id, pp1, pp2):
    answer_dict = {}
    for uuid, tok, qid, p1, p2 in zip(uuids, tokens, qa_id, pp1, pp2):
...
def test(model, data_loader, args, device):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            answer_dict_ = convert_tokens(uuids, context_tok, id.tolist(), ymin.tolist(), ymax.tolist())
            answers = dict()
            for id in answer_dict_ :
                answers[id] = answer_dict_[id].replace("~", "")
